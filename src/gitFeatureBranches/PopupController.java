...
public class PopupController
{
   ...
   public void änderungspeichern(ActionEvent event)
   {
	  ...
	  if (Tcolorpicker.getValue() == Color.LIGHTCORAL)
	  {
		 ...
		 if (Tcolorpicker.getValue() == Color.BROWN)
		 {
			...
			if (Tcolorpicker.getValue() == Color.LIGHTGREEN)
			{
			   Agenda.AppointmentImplLocal lAppointment = new Agenda.AppointmentImplLocal().withStartLocalDateTime(
						labdate
			   ).withEndLocalDateTime(lbisdate).withDescription(lnotiz).withLocation(lraum).withWholeDay(lisganztag)
			   ...
			   if (Tcolorpicker.getValue() == Color.CORNFLOWERBLUE)
			   {
				  Agenda.AppointmentImplLocal lAppointment = new Agenda.AppointmentImplLocal().withStartLocalDateTime(
						   labdate
				  ).withEndLocalDateTime(lbisdate).withDescription(lnotiz).withLocation(lraum).withWholeDay(lisganztag)
				  ...
				  if (Tcolorpicker.getValue() == Color.YELLOW)
				  {
	  ...
	  // Agenda.AppointmentImplLocal lAppointment = new
	  // Agenda.AppointmentImplLocal()
	  // .withStartLocalDateTime(LocalDate.now().atTime(4, 00))
	  // .withEndLocalDateTime(LocalDate.now().atTime(15, 30))
	  // .withDescription("It's time")
	  // .withAppointmentGroup(new
	  // Agenda.AppointmentGroupImpl().withStyleClass("group1")) // you should
	  // use a map of AppointmentGroups
	  // ;
	  // agkalender.appointments().addAll(lAppointment);
	  //
	  // getAgkalender().appointments().remove(getAppointment());
   ...
   public void löschenAusDB(Connection connection)
   {
	  ...
	  try
	  {
		 ...
		 lBefehl2.executeUpdate(
				  "DELETE FROM termin where InfoTermin ='" + getAppointment().getDescription() + "' AND  StartDatum= '"
						   + getAppointment().getStartLocalDateTime().toLocalDate() + "' AND EndDatum= '"
						   + getAppointment().getEndLocalDateTime().toLocalDate() + "';"
				  );
				  }
